...
```mermaid
graph TD
    Emb[Embedded Systems] --> Elec[Electronics]
    Emb --> Prog[Programming]
    Emb --> Arch[Architecture]
    Emb --> RTOS[Real-Time OS]
    ...
    Arch --> Micro[Microcontrollers]
    Arch --> Mem[Memory Management]
    Arch --> Int[Interrupts]

    RTOS --> Task[Task Scheduling]
    RTOS --> Conc[Concurrency]
```
